...
$stdout = Join-Path $env:TEMP ("honeypot-form-validator-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("honeypot-form-validator-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5346"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/validation-lane"; File = "02-validation-lane-proof.png"; Size = "1600,1380" },
        @{ Url = "http://127.0.0.1:$port/abuse-signals"; File = "03-abuse-signals-proof.png"; Size = "1600,1260" },
